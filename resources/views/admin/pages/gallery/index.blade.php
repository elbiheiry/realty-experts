            ...
            <div class="widget-content">
                <div class="col-sm-12">
                    <div class="add-user-form">
                        <form class="ajax-form" method="post" action="{{route('admin.gallery')}}"
                    ...
                    <div class="table-responsive">
                        <table id="datatableX" class="table table-hover table-bordered display nowrap" cellspacing="0"
                            ...
                            @foreach($images as $index => $image)
                                <tr class="ticket-tr">
                                    <td>{{$index+1}}</td>
                                    <td><img src="{{asset('storage/app/gallery/'.$image->image)}}" width="300"></td>
                                    <td>{{$image->created_at}}</td>
                                    <td>
                                        <button data-url="{{route('admin.gallery.edit',['id' => $image->id])}}"
                                                class="icon-btn green-bc btn-modal-view ">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        <button
                                                data-url="{{route('admin.gallery.delete' ,['id' => $image->id])}}"
                                                data-toggle="modal"
